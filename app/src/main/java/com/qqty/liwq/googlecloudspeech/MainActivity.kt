...
class MainActivity : AppCompatActivity(), MessageDialogFragment.Listener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<View>(R.id.saysomthing).visibility = View.VISIBLE
    }

    override fun onStart() {
        super.onStart()
        // Prepare Cloud Speech API
        bindService(Intent(this, SpeechService::class.java), mServiceConnection, Context.BIND_AUTO_CREATE)
    ...
    override fun onStop() { // Stop listening to voice
        stopVoiceRecorder()
    ...
    private val mVoiceCallback: VoiceRecorder.Callback = object : VoiceRecorder.Callback() {
        override fun onVoiceStart() {
            showStatus(true)
            runOnUiThread {
                mStatus?.text = "Listening.."
                if (mSpeechService != null) {
                    mVoiceRecorder?.sampleRate?.let { mSpeechService?.startRecognizing(it) }
                }
            }
        }
        ...
        override fun onVoiceEnd() {
            showStatus(false)
            runOnUiThread { mStatus?.text = "Listening.." }
            if (mSpeechService != null) {
    ...
    private class ViewHolder internal constructor(inflater: LayoutInflater, parent: ViewGroup?) : RecyclerView.ViewHolder(inflater.inflate(R.layout.item_result, parent, false)) {
        var text: TextView
    ...
    private class ResultAdapter internal constructor(results: ArrayList<String?>?) : RecyclerView.Adapter<ViewHolder>() {
